...
selfintro_bases = [get_eval_selfintro(example) for example in dataset["test"]]
selfintroductions = [example["selfintro"] for example in dataset["test"]]

# 결과 저장용
results = []

for messages, gt, selfintro in tqdm(zip(messages_list, selfintro_bases, selfintroductions), total=len(messages_list), desc="Generating"):
    # Chat 템플릿 적용
    ...
    results.append({"eval_selfintro": gt, "generated_text": generated_text,"selfintro": selfintro})

# CSV 저장
pd.DataFrame(results).to_csv("exaone-3.5_baseline_eval_selfintro_250531.csv", index=False, encoding="utf-8-sig")
