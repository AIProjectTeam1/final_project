...
peft_model_path = "./qlora-output_v2_explicit_guideline"
prompt_encoder_path = "./prompt-tuned_final/prompt_encoder.bin"
dataset_path = "small_resume_dataset_final"
prompt_version = "v2_explicit_guidelines_ko_improved_formated"
...
# 4. 데이터셋 준비 및 추론
raw_dataset = load_from_disk(dataset_path)["test"].shuffle(seed=42)
...
for messages, gt, resume, selfintro in tqdm(zip(messages_list, ground_truthes, resumes, selfintros), total=len(messages_list)):
    ...
    selfintro_results.append({
        "eval_selfintro": gt["eval_selfintro"],
